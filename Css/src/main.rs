//! Forge CSS language server proxy.
//!
//! Wraps vscode-css-language-server (from vscode-langservers-extracted).
...
const SERVER: &str = "vscode-css-language-server";
const ENTRY: &str = "vscode-langservers-extracted/bin/vscode-css-language-server";
...
fn run() -> i32 {
    if let Some(path) = which(SERVER) {
        eprintln!("forge-lsp-css: using {SERVER} from PATH");
    ...
    eprintln!(
        "forge-lsp-css: no CSS language server available. Tried:\n\
         - {SERVER} on PATH\n\
...
fn node_modules() -> PathBuf {
    engine_cache_dir("css").join("npm").join("node_modules")
}
...
fn install() -> bool {
    ...
    let dir = engine_cache_dir("css").join("npm");
    if dir.join(format!("node_modules/{SERVER}/package.json")).exists() {
        return true;
    }

    eprintln!("forge-lsp-css: installing {PACKAGE} via npm...");
...
fn spawn_via_node(entry: &std::path::Path) -> i32 {
    let Some(node) = which("node") else {
        eprintln!("forge-lsp-css: node not found on PATH");
        return 1;
    };
