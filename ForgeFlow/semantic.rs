...
enum Ctx {
    ...
    Action,
    Field,
    /// Start of a new statement inside a `flow`/`as`/`anders`/`terwyl`/`been`
    /// body (SPEC.md §5) — `step`/`laat`/`as`/`terwyl`/`tak`/`gee`/`elk`, not
    /// the top-level-only `gbk`/`soort`/`flow`.
    Stmt,
    /// Directly inside a `tak { }` block (SPEC.md §5.1) — only `been` is
    /// grammatically valid here, nothing else.
    TakBranch,
}

/// Finds the construct (`flow`/`as`/`anders`/`terwyl`/`tak`/`been`/`node`/
/// `soort`) whose `{ ... }` body directly encloses the cursor, by walking
/// forward through tokens before the cursor with a brace stack: each `{` is
/// tagged with the nearest preceding recognized keyword (skipping over
/// condition/param/name tokens in between, stopping at the first `{`/`}`/`;`
/// boundary it crosses without finding one), each `}` pops the stack. Returns
/// `None` at top level (empty stack) or when a `{` couldn't be attributed to
/// a known keyword (treated as opaque/unrecognized, same as top level).
/// `.fwrk`-only — `.fdgn`'s `{` bodies are handled separately (`Ctx::Field`).
fn enclosing_construct<'a>(toks: &'a [Tok], line: usize, character: usize) -> Option<&'a str> {
    const TAGS: &[&str] =
        &["flow", "as", "anders", "terwyl", "tak", "been", "node", "soort"];
    let mut stack: Vec<&'a str> = Vec::new();
    for (i, t) in toks.iter().enumerate() {
        if t.line > line || (t.line == line && t.col >= character) {
            break;
        }
        if t.text == "{" {
            ...
            while j > 0 {
                ...
                if let Some(tag) = TAGS.iter().find(|k| **k == w) {
                    kind = Some(tag);
                    break;
                }
                if w == "{" || w == "}" || w == ";" {
...
pub fn completions_for(line: usize, character: usize, toks: &[Tok], mode: FileMode) -> Vec<Value> {
    ...
    // Deliberately NOT restricted to `t.line == line`: a token on an earlier
    // line (e.g. the `{` that opened the current, still-empty line) is still
    // the relevant preceding token and must be tracked, or `prev` stays
    // `None` on every blank line and falls into the no-prev-token fallback
    // below instead of the real context.
    let mut prev: Option<&Tok> = None;
    let mut prev2: Option<&Tok> = None;
    let mut prev3: Option<&Tok> = None;
    for t in toks {
        let ends_after_cursor = t.line > line || (t.line == line && t.col + t.len > character);
        if ends_after_cursor {
            break;
        }
        prev3 = prev2;
        prev2 = prev;
    ...
    if let Some(p) = prev {
        match p.text.as_str() {
            ...
            ";" | "{" | "}" | ")" => {
                ctx = if mode == FileMode::Fwrk {
                    match enclosing_construct(toks, line, character) {
                        Some("tak") => Ctx::TakBranch,
                        Some(_) => Ctx::Stmt,
                        None => Ctx::Keyword,
                    }
                } else {
    ...
    match ctx {
        ...
        Ctx::Stmt => {
            ...
            for k in &["step", "laat", "as", "terwyl", "tak", "gee", "elk"] {
                push_item(&mut items, &mut seen, k, 14, "keyword");
            }
        }
        Ctx::TakBranch => {
            push_item(&mut items, &mut seen, "been", 14, "keyword");
        }
    }
    items
}
...
// ── Document highlight (SPEC.md naming table, agreed design) ────────────────
//
// Two matching strategies:
//   - Whole-file, by name text: `flow`/`soort`/`node`/`tak` declarations,
//     `soort` fields, `gbk` imports, and call/reference sites — anything
//     tagged `Role::Function` at its use, matched against every other
//     `Role::Function` token with the same text in the file.
//   - Scoped to the enclosing `flow`: parameters, `laat` variables, `.elk`
//     loop variables — anything tagged `Role::Variable`/`Role::Property`,
//     matched by name only within that flow's own token range (its `flow`
//     keyword through the matching closing `}`, so parameters declared
//     before the body `{` are included). Falls back to whole-file when
//     there's no enclosing flow (e.g. a `soort` field, or `.fdgn` mode,
//     which has no flows at all) — this also happens to be the right
//     behavior for `soort` fields, which are meant to match project-wide.
//
// Text-based throughout — same "structure only" policy as the rest of this
// parser (see SPEC.md's opening note). Two different flows each declaring
// `laat x = ...` cannot collide (scoped to their own flow's token range);
// two *unrelated* same-named `Role::Function` declarations elsewhere in the
// file (e.g. a node and a flow accidentally sharing a name) are treated as
// the same symbol — a known, accepted approximation, not cross-checked
// against declaration kind.
...
/// Token-index range `[start, end]` of the `flow` enclosing `line`/
/// `character` — from the `flow` keyword itself (so its parameter list is
/// included) through the matching closing `}` of its body. `None` if the
/// position isn't inside any flow.
fn enclosing_flow_range(toks: &[Tok], line: usize, character: usize) -> Option<(usize, usize)> {
    let mut i = 0;
    while i < toks.len() {
        if toks[i].kind == Kind::Kw && toks[i].text == "flow" {
            let mut j = i + 1;
            while j < toks.len() && toks[j].text != "{" {
                j += 1;
            }
            if j >= toks.len() {
                return None; // malformed tail — nothing more to scan
            }
            let mut depth = 0i32;
            let mut k = j;
            loop {
            ...
            let end = k;
            let (sl, sc) = (toks[i].line, toks[i].col);
            let (el, ec) = (toks[end].line, toks[end].col + toks[end].len);
            let at_or_after_start = (line, character) >= (sl, sc);
            let at_or_before_end = (line, character) <= (el, ec);
...
pub fn document_highlight_at(
    ...
    let Some(hit) = sems
        .iter()
        .find(|s| s.line == line && character >= s.col && character <= s.col + s.len)
    else {
        return Vec::new();
    };
    let Some(tok) = toks.iter().find(|t| t.line == hit.line && t.col == hit.col) else {
        return Vec::new();
    };
    ...
    let scoped_roles = matches!(hit.role, Role::Variable | Role::Property);
    let bounds = if scoped_roles {
        enclosing_flow_range(toks, line, character)
    } else {
        None
    };

    // O(1)-ish role lookup by (line, col) — every meaningfully-classified
    // token has exactly one SemToken at its start position.
    let role_at = |l: usize, c: usize| sems.iter().find(|s| s.line == l && s.col == c).map(|s| s.role);
    ...
    toks.iter()
        ...
        .filter(|(idx, t)| {
            ...
            match role_at(t.line, t.col) {
                Some(Role::Function) => !scoped_roles,
                Some(Role::Variable) | Some(Role::Property) => scoped_roles,
                _ => false,
            }
        })
        .map(|(_, t)| HighlightRange { line: t.line, col: t.col, len: t.len })
        .collect()
}
